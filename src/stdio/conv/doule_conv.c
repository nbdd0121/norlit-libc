...
static inline uint64_t roundedSHR(uint64_t operand, uint8_t count) {
    operand >>= (count - 1);
    operand++;
    operand >>= 1;
    return operand;
...
LongDouble construct_long_double(double val) {
    ...
    u.d = val;
    uint64_t bits = u.i;
    self.exp = (bits >> 52) & 0x7FF;
    self.frac = bits & 0xFFFFFFFFFFFFFULL;
    if (self.exp == 0) {
        self.exp = 1 - (1023 + 52);
    } else {
        self.exp -= 1023 + 52;
        self.frac += 0x10000000000000ULL;
    }
    return self;
}
...
static LongDouble minus(LongDouble a, LongDouble b) {
    assert(a.exp == b.exp && a.frac > b.frac);
    return (LongDouble) { a.frac - b.frac, a.exp };
}

static LongDouble multiply(LongDouble a, LongDouble b) {
    uint64_t aLo = a.frac & 0xFFFFFFFF, aHi = a.frac >> 32,
...
static double extract(LongDouble self) {
    ...
    if (exp <= 0) {
        ...
        u.i = ((uint64_t)(exp) << 52) | frac;
    }
    return u.d;
}

// This is from Florian Loitsch's sample implementation
static LongDouble getPower(int k) {
...
static int calculateK(int e, int alpha, int gamma) {
    return ceil((alpha - e + 63) * 0.30102999566398114);
}

static uint64_t longDoubleToInteger(LongDouble D) {
    if (D.exp > 0) {
        return D.frac << D.exp;
    } else {
        return D.frac >> -D.exp;
...
void desemble_double(double v, uint64_t* s, int* n, int* k) {
    ...
    lowerBoundary = multiply(lowerBoundary, c_mk);
